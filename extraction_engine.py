...
class UrlValidator:
    ...
    def valid(self, token):
        ...
        if lower.startswith("mailto:"):
            return bool(EMAIL.fullmatch(token[7:]))
        if lower.startswith("tel:"):
            return bool(re.fullmatch(r"(?i)tel:\+?[0-9][0-9(). -]{5,}[0-9]", token))
        ...
        if any(not re.fullmatch(r"(?i)[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?", label) for label in labels):
            return False
        suffix = self.psl(ascii_host).suffix
        return bool(suffix) or (has_scheme and len(labels) == 1)
...
class PdfExtractionEngine:
        ...
        for widget in page.widgets() or []:
            ...
            for token, _, _ in self.validator.candidates(str(widget.field_value or "")):
                target = next((
                    ...
                    and "TEXT" in item.sources and self.boxes_match(item.bbox, bbox)
                ), None)
                if target is not None and "PDF_FORM" not in target.sources:
    ...
    def ocr_regions(self, page, page_image, scale, searchable, native_rects, page_images, vector_drawings):
        ...
        if not page_images and not vector_drawings:
            return []

        width, height = page_rect.width, page_rect.height
        regions = [
            fitz.Rect(0, 0, width, height * 0.20),
            fitz.Rect(0, height * 0.80, width, height),
            fitz.Rect(0, height * 0.20, width * 0.15, height * 0.80),
            fitz.Rect(width * 0.85, height * 0.20, width, height * 0.80),
        ]
        ...
        for image in page_images:
            for rect in page.get_image_rects(image[0]):
                if rect.width * rect.height < page_area * 0.60:
                    regions.append(rect)
        native_boxes = [fitz.Rect(rect) for rect in native_rects]
        for drawing in vector_drawings:
            rect = fitz.Rect(drawing.get("rect"))
            if (
                20 <= rect.width <= width * 0.7
                and 6 <= rect.height <= height * 0.25
                and not any(rect.intersects(native_rect) for native_rect in native_boxes)
            ):
                regions.append(rect + (-5, -5, 5, 5))
        unique = []
        seen = set()
        for rect in regions:
            rect = fitz.Rect(rect) & page_rect
            key = tuple(round(value, 1) for value in rect)
            if not rect.is_empty and key not in seen:
                seen.add(key)
                unique.append(rect)
            if len(unique) >= 6:
                break
        hits = []
        for rect in unique:
            # Searchable pages get one cached 300-DPI pass per bounded region;
            # expensive retries are reserved for image-only full-page fallback.
    ...
    def process_page(self, get_document, page_index, file_id):
        ...
        master_tokens = {item.token for item in occurrences}
    ...
    def extract(self, pdf_path, file_id, progress_state):
        ...
        try:
            with ThreadPoolExecutor(max_workers=workers) as executor:
                futures = {
                    executor.submit(self.process_page, get_document, index, file_id): index
