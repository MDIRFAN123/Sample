...
progress_lock = threading.Lock()  # protects progress reads and worker updates.
...
def process_pdf(pdf_path: str, file_id: str) -> dict:
    ...
    return engine.extract(
        pdf_path, file_id, progress[file_id], progress_lock=progress_lock
    )


def run_processing_job(pdf_path: str, file_id: str) -> None:
    logger.info("Scan started: %s", file_id)
    result = None
    error_message = None
    try:
        result = process_pdf(pdf_path, file_id=file_id)
        logger.info("Worker completed: %s", file_id)
    except Exception as exc:
        error_message = str(exc).strip() or exc.__class__.__name__
        logger.exception("Worker failed: %s: %s", file_id, error_message)
    finally:
        logger.info("Clearing processing state: %s", file_id)
            ...
            with progress_lock:
                if error_message is not None:
                    ...
                    progress[file_id].update({
                        ...
                        "elapsed_seconds": result["scan_time_seconds"],
                        "done": True,
                        "status": "COMPLETED",
                        "error": None,
                        "result": result,
...
def api_process(file_id):
    ...
    # Start one background worker. Repeated AE/browser requests for the same
    # active job are idempotent and continue observing the same progress state.
    with processing_lock:
        with progress_lock:
            ...
            if status == "COMPLETED" and state.get("result") is not None:
                return jsonify(state["result"]), 200
            if status == "FAILED":
                return jsonify({"error": state.get("error") or "Scan failed"}), 500
        ...
        worker = threading.Thread(
            ...
            args=(pdf_path, file_id),
            daemon=True,
            name=f"qr-scan-{file_id[:8]}",
        )
        try:
            worker.start()
        except Exception as exc:
            ...
            processing_files.discard(file_id)
            logger.exception("Worker failed to start: %s", file_id)
...
def get_progress(file_id):
    ...
    with progress_lock:
        stored_state = progress.get(file_id)
        state = dict(stored_state) if stored_state is not None else None
    if state is None:
        return jsonify({"error": "Scan not found"}), 404
