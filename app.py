...
def detect_qr_multi(img_bgr: np.ndarray, thorough=True):
    ...
    # The page is rendered only once at 300 DPI.  If native decoding fails,
    # bicubic upscaling supplies the 600-DPI-equivalent small-code pass without
    # asking MuPDF to render the page again.
    for factor in (1, 2):
        base = img_bgr if factor == 1 else cv2.resize(
        ...
        variants = (base, clahe, threshold, sharpen)
        rotations = (None, cv2.ROTATE_90_CLOCKWISE, cv2.ROTATE_180,
                     cv2.ROTATE_90_COUNTERCLOCKWISE)
        for variant in variants:
            for rotation in rotations:
                candidate = variant if rotation is None else cv2.rotate(variant, rotation)

                def restore(points, code=rotation, scale=factor):
