...
from extraction_engine import PdfExtractionEngine, OcrEngine
...
def detect_qr_multi(img_bgr: np.ndarray, thorough=True):
    ...
    sharpened = cv2.addWeighted(
        enhanced, 1.8, cv2.GaussianBlur(enhanced, (0, 0), 1.0), -0.8, 0
    )
    _, binary = cv2.threshold(
        sharpened, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
    )

    def decode_variant(image, points_match_original=False, point_mapper=None):
        decoded = []
        if hasattr(qr, "detectAndDecodeMulti"):
            ...
            if ok and data_list is not None:
                for index, data in enumerate(data_list):
                    pts = points[index] if points is not None else None
                    if pts is not None and not points_match_original:
        ...
        if data:
            if points is not None and not points_match_original:
    ...
    # Process variants sequentially to limit memory on high-DPI pages.
    decode_variant(img_bgr, True)
    if out:
        return out
    decode_variant(enhanced, True)
    if out:
        return out
    decode_variant(binary, True)
    if out:
        return out
    ...
    for rotation in (
        ...
        rotated = cv2.rotate(img_bgr, rotation)

        def restore_points(points, rotation_code=rotation):
            restored = np.array(points, dtype=np.float32).reshape(-1, 2).copy()
            x_values, y_values = restored[:, 0].copy(), restored[:, 1].copy()
            if rotation_code == cv2.ROTATE_90_CLOCKWISE:
                restored[:, 0] = y_values
                restored[:, 1] = original_height - 1 - x_values
            elif rotation_code == cv2.ROTATE_180:
                restored[:, 0] = original_width - 1 - x_values
                restored[:, 1] = original_height - 1 - y_values
            else:
                restored[:, 0] = original_width - 1 - y_values
                restored[:, 1] = x_values
            return restored

        decode_variant(rotated, point_mapper=restore_points)
        del rotated
        if out:
            return out
...
def extract_url_from_qr_payload_exact(payload: str):
    if not payload:
        return None
    matches = extract_links_exact_with_pos(payload.strip())
...
def detect_qr_with_preview(
    ...
    for d, pts in detect_qr_multi(img_bgr, thorough=thorough):
        pay = (d or "").strip()
        if not pay:
            continue
