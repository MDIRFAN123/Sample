...
def create_app(settings: Settings | None = None) -> Flask:
    ...
    def instruction_path(team: str) -> Path | None:
        folder = "cops" if team == settings.team_app else None
    ...
    def index():
        return render_template(
            "index.html",
            cops_rovo_url=settings.cops_rovo_url,
            cops_gpt_url=settings.cops_gpt_url,
            team_app=settings.team_app,
    ...
    @app.get("/team_config")
    def team_config():
        return jsonify({settings.team_app: TEAM_CONFIG[settings.team_app]})
    ...
    def preview_cops_dsr():
        ...
        return jsonify(filename=path.name, paragraphs=paragraphs, tables=tables, blocks=blocks)
    ...
    def fetch_workbook():
        ...
        try:
            ...
            if settings.enterprise_mode or not settings.auto_upload_enabled:
                return jsonify(
                    status="monitor_required",
                    message=(
                        "The workbook download is ready. After it downloads, "
                        "upload the workbook manually to continue."
                    ),
    ...
    def start_workbook_monitor():
        if settings.enterprise_mode or not settings.auto_upload_enabled:
            return jsonify(error="Downloads-folder monitoring is disabled in enterprise mode."), 409
        payload = request.get_json(silent=True) or {}
        project_reference = str(payload.get("project_reference", "")).strip()
        task_number = str(payload.get("task_number", "")).strip()
