...
def create_app(settings: Settings | None = None) -> Flask:
    ...
    def upload_workbook():
        ...
        try:
            ...
            workfront_content = (
                database.fetch_workfront_content(project_reference, task_number)
                if project_reference and task_number
                else {}
            )
            return jsonify(build_session_payload(workbook_path, workfront_content, "manual"))
        except (DatabaseConfigurationError, WorkbookError) as error:
    ...
    def poll_workbook_monitor():
        ...
        if monitor.get("detected_path"):
            ...
            try:
                ...
                result = build_session_payload(detected, task.workfront_content, "detected")
                monitor_sessions.pop(monitor_id, None)
