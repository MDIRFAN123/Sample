...
def create_app(settings: Settings | None = None) -> Flask:
    ...
    def prepare_cops():
        ...
        try:
            ...
            dsr_document = Document(result.path)
            dsr_document.add_heading("COPS Request Details", level=1)
            dsr_document.add_paragraph(f"Project Reference Number: {project}")
            dsr_document.add_paragraph(f"Task Number: {task_number}")
            dsr_document.save(result.path)
    ...
    def cops_data_safety():
        ...
        try:
            ...
            masked_workbook_name = f"WB_Masked_{Path(original_name).stem}{workbook_path.suffix.lower()}"
