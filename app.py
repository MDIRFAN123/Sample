...
from services.workbook_media import normalize_wmf_images
from services.dsr_service import DSRConfigurationError, DSRService
from services.package_service import create_zip, package_directory, retain_workbook as retain_package_workbook
...
def create_app(settings: Settings | None = None) -> Flask:
    ...
    def build_session_payload(source: Path, workfront_content: dict, upload_source: str) -> dict:
    ...
    def prepare_cops():
        ...
        try:
            ...
            return jsonify(
                ...
                message="Workbook details found.",
            )
    ...
    def create_cops_dsr():
        ...
        workbook_files = list(target.glob("*.xlsx")) + list(target.glob("*.xlsm"))
        if not workbook_files:
            return jsonify(error="The workbook must finish downloading before the DSR is created."), 409
        try:
            result = dsr_service.generate_dsr(project, target / "DSR_Report.docx")
            return jsonify(status="ready", dsr=result.public_data, message="DSR report created.")
        except (DatabaseConfigurationError, DSRConfigurationError):
            app.logger.warning("COPS DSR configuration is incomplete")
            return jsonify(error="DSR generation failed. Please retry or contact the administrator."), 422
        except Exception:
            app.logger.exception("COPS DSR creation failed")
            return jsonify(error="Unable to create the COPS DSR report."), 500
    ...
    def download_cops_dsr():
        ...
        return send_file(path, as_attachment=True, download_name="DSR_Report.docx")
    ...
    def preview_cops_dsr():
        ...
        if not path.is_file():
            return jsonify(error="DSR report was not found."), 404
        try:
            return jsonify(title="DSR Report", content=document_text(path))
        except Exception:
            app.logger.exception("COPS DSR preview failed")
            return jsonify(error="The DSR report could not be previewed."), 500

    @app.get("/sheet_instructions/<team>")
    def team_sheet_instructions(team: str):
        team_key = team.strip().lower().replace("-", "_").replace(" ", "_")
        if team_key not in {"cops", "mqa", "channel_marketing"}:
            return jsonify(error="Unknown team."), 404
        path = Path(app.root_path) / "sheet_instructions" / team_key / "sheet_instructions.txt"
        if not path.is_file():
            return jsonify(error="Sheet instructions were not found."), 404
        return jsonify(team=team_key.replace("_", " ").title(), content=path.read_text(encoding="utf-8"))
    ...
    def cops_data_safety():
        ...
        target = package_directory(settings.monitored_download_dir, project, task_number)
        workbook_files = list(target.glob("*.xlsx")) + list(target.glob("*.xlsm"))
        dsr_path = target / "DSR_Report.docx"
        if not workbook_files or not dsr_path.is_file():
            return jsonify(error="Workbook and DSR must both be ready for Data Safety validation."), 409
        try:
            workbook = load_workbook(workbook_files[0], read_only=True, data_only=True)
            try:
                workbook_text = "\n".join(
                ...
                workbook.close()
            dsr_text = document_text(dsr_path)
            instructions = (Path(app.root_path) / "sheet_instructions" / "cops" / "sheet_instructions.txt").read_text(encoding="utf-8")
            return jsonify(pii_scanner.scan_sections({
                "DSR Report": dsr_text,
                "Workbook Content": workbook_text,
                "Sheet Instructions": instructions,
            }, "COPS"))
        except Exception:
            app.logger.exception("COPS Data Safety validation failed")
            return jsonify(error="Data Safety validation could not be completed."), 500

    @app.get("/cops/package")
    def download_cops_package():
        project = str(request.args.get("project_reference", "")).strip()
        task_number = str(request.args.get("task_number", "")).strip()
        target = package_directory(settings.monitored_download_dir, project, task_number)
        workbook_files = list(target.glob("*.xlsx")) + list(target.glob("*.xlsm"))
        if not (target / "DSR_Report.docx").is_file() or not workbook_files:
            return jsonify(error="Both the workbook and DSR must be ready before downloading the package."), 409
        archive = create_zip(target)
    ...
    def upload_workbook():
        ...
        try:
            ...
            workfront_content = (
                database.fetch_workfront_content(project_reference, task_number or None)
                if project_reference and (task_number or team == "COPS")
                else {}
            )
            result = build_session_payload(workbook_path, workfront_content, "manual")
            if team == "COPS" and project_reference:
                target = package_directory(settings.monitored_download_dir, project_reference, task_number)
                retained = retain_package_workbook(workbook_sessions[result["workbook_session_id"]]["source"], target)
                result.update(
