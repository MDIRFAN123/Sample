...
def create_app(settings: Settings | None = None) -> Flask:
    ...
    def cops_data_safety():
        payload = request.get_json(silent=True) or {}
        session = workbook_sessions.get(str(payload.get("workbook_session_id", "")))
        project = str(payload.get("project_reference", "")).strip()
        task_number = str(payload.get("task_number", "")).strip()
        ...
        if not session or not dsr_path.is_file() or not instructions or not instructions.is_file():
            return jsonify(error="DSR Report, workbook content, and sheet instructions must all be ready."), 409
        from docx import Document
        from openpyxl import load_workbook
        document = Document(dsr_path)
        dsr_parts = [p.text for p in document.paragraphs]
        dsr_parts.extend(cell.text for table in document.tables for row in table.rows for cell in row.cells)
        workbook = load_workbook(session["source"], read_only=True, data_only=False)
        try:
            workbook_text = "\n".join(
                str(value) for sheet in workbook.worksheets for row in sheet.iter_rows(values_only=True)
        ...
        sections = {
            ...
            "Sheet Instructions": instructions.read_text(encoding="utf-8"),
        }
        return jsonify(pii_scanner.scan_sections(sections, "Complete COPS content"))
