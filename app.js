...
const AppState = {
    ...
    temporaryWorkbookFile: null,
    generatedPromptFile: null,
    readyHighlightKey: "",
...
function registerBaseEvents() {
    ...
    UI.temporaryWorkbookAttachment?.addEventListener(
        "dragstart",
        handlePromptAttachmentsDrag
    );
    ...
    UI.copyPromptAttachmentButton?.addEventListener(
        "click",
        copyPrompt
    );
...
async function renderTemporaryWorkbookAttachment() {
    ...
    try {
        ...
        const response =
            await fetch(attachment.download_url);
        if (!response.ok)
            throw new Error("Unable to prepare draggable workbook.");
        const blob = await response.blob();
        AppState.temporaryWorkbookFile =
            new File(
                [blob],
                attachment.filename,
                {
        ...
        console.error(error);
        AppState.temporaryWorkbookFile = null;
...
function clearTemporaryWorkbookAttachment() {
    ...
    AppState.temporaryWorkbookFile = null;
    AppState.generatedPromptFile = null;
...
function handlePromptAttachmentsDrag(event) {
    ...
    if (files.length < 2) {
        ...
        event.preventDefault();
        showToast("Attachments are still being prepared.");
        return;
    ...
    if (files.length) {
        ...
        try {
            ...
            console.warn(
                "This browser does not support dragging generated files.",
                error
            );
