...
class WorkbookProcessor:
    ...
    def _load_mapping(cls) -> dict:
        if cls._mapping_cache is None:
            try:
                cls._mapping_cache = json.loads(
                    cls.MAPPING_PATH.read_text(encoding="utf-8")
                )
    ...
    def resolve_worksheet_template(cls, sheet_name: str) -> dict:
        ...
        matches = []
        configured_mappings = mapping.get("templates", {})
        if not isinstance(configured_mappings, dict):
            configured_mappings = {}
        for template_key, template in templates.items():
            keywords = configured_mappings.get(template_key, [])
        ...
        if fallback not in templates:
            raise WorkbookError(
                "Validation template is unavailable. Please contact the administrator."
            )
