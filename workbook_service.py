...
class WorkbookProcessor:
    ...
    PROMPT_TEMPLATE_PATH = Path(__file__).resolve().parents[1] / "prompt.txt"
    GENERIC_WORKSHEET_TYPE = "Generic Workbook"
    WORKSHEET_TYPES = (
        ("Waterfall", (("waterfall",),)),
        ("Offer Description", (("offer", "description"), ("offer", "desc"), ("offer", "details"))),
        ("Landing Page", (("landing", "page"), ("landing", "pages"))),
        ("Terms", (("terms",),)),
        ("Email", (("email",),)),
    )
    DEFAULT_VALIDATION_RULES = (
        "Validate worksheet structure, required fields, data types, formulas, and values.\n"
        "Flag missing, malformed, duplicate, or contradictory values.\n"
        "Report each issue with its row or cell reference, severity, evidence, and recommended remediation."
    )
    ...
    def build_payload(self, workbook_path: Path, workfront_content: dict | str | None = None) -> dict:
        ...
        try:
            for sheet in workbook.worksheets:
                ...
                worksheet_type = self.detect_worksheet_type(sheet.title)
                sheet_data[sheet.title] = {
                    "worksheet_type": worksheet_type,
                    "validation_summary": self._validation_summary(worksheet_type),
    ...
    def detect_worksheet_type(cls, sheet_name: str) -> str:
        words = set(cls.normalize_sheet_name(sheet_name).split())
        for worksheet_type, keyword_groups in cls.WORKSHEET_TYPES:
            if any(set(group).issubset(words) for group in keyword_groups):
                return worksheet_type
        return cls.GENERIC_WORKSHEET_TYPE

    @classmethod
    def _sheet_instructions(cls, sheet_name: str, worksheet_type: str) -> str:
        validation_rules = cls._validation_rules(worksheet_type)
        template = cls.PROMPT_TEMPLATE_PATH.read_text(encoding="utf-8").strip()
    ...
    @classmethod
    def _validation_rules(cls, worksheet_type: str) -> str:
        return (
            cls._load_sheet_text(cls.RULES_DIR, worksheet_type)
            or cls._load_sheet_text(cls.RULES_DIR, "generic")
            or cls.DEFAULT_VALIDATION_RULES
        )

    @classmethod
    def _validation_summary(cls, worksheet_type: str) -> list[str]:
        rules = cls._validation_rules(worksheet_type)
        bullets = [
        ...
        generic_bullets = [
            ...
            for sentence in re.split(
                r"(?:\r?\n)+|(?<=\.)\s+", cls.DEFAULT_VALIDATION_RULES
            )
            if sentence.strip()
        ]
    ...
    def _generated_prompt(
        cls, offer: str, name: str, sheet: str, worksheet_type: str,
        dimensions: str, sample: str
    ) -> str:
        instructions = cls._sheet_instructions(sheet, worksheet_type)
        prompt = (
